...
public class EmgData {
    ...
    public String getLine_withJumps() {
        StringBuilder return_SB = new StringBuilder();
        for (int i_emg_num = 0; i_emg_num < 8; i_emg_num++) {
            return_SB.append(String.format("%f,\n", emgData.get(i_emg_num)));
        }
        return return_SB.toString();
    }
    ...
    // Implementación KUMATO
    public Double[] getDouble_EmgData(){
        Double[] emgDouble = new Double[8];
        for (int i = 0; i < 8; i++) {
            emgDouble[i] = emgData.get(i);
        }
        return emgDouble;
    }
}
